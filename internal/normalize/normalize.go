...
// NFKCWithOffsets returns the compatibility-composed spelling of s and maps
// each rune boundary in that spelling back to a byte offset in s. Expansions
// map to one original position, exactly as ICU's dictionary break engine does.
func NFKCWithOffsets(s string) (string, []int) {
	type mappedRune struct {
		r      rune
		origin int
	}
	var mapped []mappedRune
	for i := 0; i < len(s); {
		start := i
		r, size := wtf8.DecodeRune(s[i:])
		i += size
		decomposed := appendDecomposed(nil, r, true)
		for _, c := range decomposed {
			mapped = append(mapped, mappedRune{r: c, origin: start})
		}
	}

	// Canonical ordering is stable and carries source positions with the marks.
	for i := 0; i < len(mapped); {
		if combiningClass(mapped[i].r) == 0 {
			i++
			continue
		}
		j := i
		for j < len(mapped) && combiningClass(mapped[j].r) != 0 {
			j++
		}
		sort.SliceStable(mapped[i:j], func(a, b int) bool {
			return combiningClass(mapped[i+a].r) < combiningClass(mapped[i+b].r)
		})
		i = j
	}

	// Combining marks are in the same normalization chunk as their starter.
	starterOrigin := -1
	for i := range mapped {
		if combiningClass(mapped[i].r) == 0 {
			starterOrigin = mapped[i].origin
		} else if starterOrigin >= 0 {
			mapped[i].origin = starterOrigin
		}
	}
	...
	if len(mapped) > 0 {
		out := mapped[:1]
		starter := 0
		lastClass := combiningClass(mapped[0].r)
		if lastClass != 0 {
			lastClass = 256
		}
		for _, c := range mapped[1:] {
			class := combiningClass(c.r)
			if composed := composePair(out[starter].r, c.r); composed != 0 &&
				(lastClass < class || lastClass == 0) {
				out[starter].r = composed
				continue
			}
			if class == 0 {
				starter = len(out)
	...
	var b strings.Builder
	b.Grow(len(s))
	offsets := make([]int, 0, len(mapped)+1)
	buf := make([]byte, 0, 4)
	for _, c := range mapped {
		offsets = append(offsets, c.origin)
		buf = wtf8.AppendRune(buf[:0], c.r)
		b.Write(buf)
	}
...
// CombiningClass is the canonical combining class of r.
func CombiningClass(r rune) int { return combiningClass(r) }
